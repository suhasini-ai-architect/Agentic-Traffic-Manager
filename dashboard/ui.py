...
def fetch_live_metrics():
    try:
        with Session(engine) as session:
            ...
            return {
                "status": "Healthy", "blocked": blocked, "cache_rate": cache_rate,
                "avg_lat": f"{avg_latency:.3f}s", "raw_logs": raw_logs
            }
...
with col1: st.metric("System Fabric Status", metrics["status"])
with col2: st.metric("Security Violations Blocked", metrics["blocked"])
with col3: st.metric("FinOps Cache Hit Rate", metrics["cache_rate"])
with col4: st.metric("Average Proxy Latency", metrics["avg_lat"])
...
with left_col:
    ...
    if user_input:
        ...
        with st.status("Mesh Interceptor Evaluating Payload...", expanded=True) as status:
            try:
                # Forward state configurations dynamically down to your gateway daemon instance
                payload = {
                    "session_id": "session-corp-prod-abc", 
                    "prompt": user_input,
                ...
                if response.status_code == 200:
                    ...
                    if res_data.get("status") == "blocked":
                        ...
                        with st.chat_message("assistant"):
                            st.error(f"**Mesh Security Alert:** Request terminated. Reason: {res_data.get('reason') or 'Policy Overrides Detected.'}")
                    else:
                        status.update(label="✅ Payload Cleared", state="complete")
                        with st.chat_message("assistant"):
                ...
                st.error("Could not connect to the Guardian-Mesh Gateway daemon.")

with right_col:
    st.subheader("📋 Direct Audit Ledger (`SQLModel` Sync)")
    if metrics["raw_logs"]:
        st.dataframe(pd.DataFrame(metrics["raw_logs"]), use_container_width=True, hide_index=True)
    else:
